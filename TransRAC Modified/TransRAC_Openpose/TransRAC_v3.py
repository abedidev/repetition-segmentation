...
class attention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        # ------------------------------------------- #
        # vars, means = torch.var_mean(scores, dim=-1)
        torch.nn.functional.normalize(scores, p=2.0, dim=-1, eps=1e-12, out=None)
        # means.unsqueeze_(-1)
        # vars.unsqueeze_(-1)
        # scores -= means
        # scores /= vars
        ...
        # print('>>> before softmax in attention <<<')
        # print(f'scores.shape = {scores.shape}')
        # print(scores)
        scores = self.softmax(scores)
        scores = self.dropout(scores)  # [B,head, F, F]
        # print('>>> after softmax in attention <<<')
        # print(f'scores.shape = {scores.shape}')
        # print('attention score of 1st frame',scores[0][0][0])
...
class Similarity_matrix(nn.Module):
    ...
    def __init__(self, input_size,num_heads=4, model_dim=512):
        super().__init__()
...
class TransferModel(nn.Module):
    def __init__(self, checkpoint, scales, num_frames):
        ...
        self.sims = Similarity_matrix(input_size=75)
        self.conv3x3 = nn.Conv2d(in_channels=4,  # num_head*scale_num
                                 out_channels=32,
                                 kernel_size=3,
                                 padding=1)
        ...
        self.bn = nn.BatchNorm1d(num_frames)

    def forward(self, x):
        # x : tensor([batch_size,num_frames,
        # x: tensor([batch_size, channel, temporal_dim, height, width])
        with autocast():
            batch_size, num_frames, nf = x.shape
            x = self.bn(x)
            ...
            # slice = []
            ## feature extract with video SwinTransformer                        for crop in crops:
            ...
            x_scale = x
                ...
                # -------- similarity matrix ---------
            x_sims = self.sims(x_scale, x_scale, x_scale)  # -> [b,4,f,f]
            #print(f'>>>> {x_sims.shape} <<<<')

            #print(f'X after similiarity matrix = \n  {x_sims}')

            # x = torch.cat(multi_scales, dim=1)  # [B,4*scale_num,f,f]
            ## x are the similarity matrixs
            x_matrix = x_sims
            x_conv = self.conv3x3(x_matrix)
            #print(f'>>>> {x_conv.shape} <<<<')
            x = F.relu(self.bn2(x_conv))  # [b,32,f,f]
            # print(f'X after bn2 and conv2d =  {x}')
            x = self.dropout1(x)
