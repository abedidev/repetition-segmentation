...
class datasetHandler():
    ...
    def createDataset(self, exercise):
        with open("Exercises/"+str(exercise)+"/NO_REPCOUNTS.txt", "w") as ns:
            pass
    ...
    def appendToDataset(self, exercise, video):
        ...
        if exists("Annotations/Exercise_"+str(exercise)+"/DensityMaps/V"+str(video)+"_DM.csv"):
            with open("Annotations/Exercise_"+str(exercise)+"/DensityMaps/V"+str(video)+"_DM.csv", "r") as rp:
                dMap = rp.readline()
                dMap = dMap.split("\n")[0]
                dMap = [float(x) for x in dMap.split(",")]
                repCount = round(sum(dMap))
                #print(repCount)
        if repCount == -1:
            repCount = None
        if repCount == None:
            with open("Exercises/"+str(exercise)+"/NO_REPCOUNTS.txt", "a") as ns:
            ...
            with open("Exercises/"+str(exercise)+"/master_dataset.txt", "a") as md:
                with open("Exercises/"+str(exercise)+"/videos/V"+str(video)+"/Features.txt", "r") as f:
                    ...
                    for line in f:
                        ...
                        frameSequence.append(features)
                    if len(frameSequence) != 0:
                        md.write("V"+str(video)+":"+person+":"+str(len(frameSequence))+":"+str(repCount)+":"+str(frameSequence)+"\n")
...
class LSTMdataset(Dataset):
    ...
    def __init__(self, folder, filename):
        ...
        self.y_data = torch.from_numpy(xy[:, [3]].astype(np.float32)).to(self.device) # size [n_samples, 1]
...
class LSTMdatasetWrapper(Dataset):
    def __init__(self, folder, filename):
        ...
        with open(folder+"/"+filename, "r") as od:
            for line in od:
                with open(folder+"/"+filename+"_dummyDataset_"+str(count)+".txt", "w") as dd:
                    dd.write(line)
                    count += 1
        self.datasets = []
        for i in range(count):
            self.datasets.append(LSTMdataset(folder, filename+"_dummyDataset_"+str(i)+".txt"))
            os.remove(folder+"/"+filename+"_dummyDataset_"+str(i)+".txt")
